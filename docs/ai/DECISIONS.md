...
### 補：theme 揀選收埋（Penny 頭版 review）

第一版三粒掣直接攤喺頭版。Penny 一睇就講：**擺喺正面、仲佔咁大篇幅，好奇怪。** 啱——
theme 係一個**偏好設定**，一個玩家一世可能只撳一次嘅嘢，唔應該同十三隻遊戲爭注意力。

收埋做一粒掣，撳先展開；三套各自用返自己嗰套形態（機舖係投幣掣、雜誌係報頭角落
一行 `Editorial edition ▾`、調度台係 rail 上面一粒 `MODE` 鍵）。**收埋唔等於收起**：
三粒 `[data-theme-value]` 一直喺 DOM，展開之後仍然係 44px native button、Tab 到、
Space 撳得郁；撳出面同 Esc 都收得返（一塊收唔返嘅面板同一直攤開冇分別）。

把尺跟住改，而且**改完之後守得多咗**：本來喺收埋咗嘅元素度叫 `focus()` 就當數
——`display: none` 嘅嘢 focus 唔到，會靜靜雞報 false，而條 check 睇落好似仲喺度守緊。
而家問返真正嗰條鍵盤路：Tab 到粒掣 → Space 展開 → Tab 入去 → Space 揀。另外加咗
一條「頭版預設收埋」（一粒 toggle、零塊展開嘅面板、零粒見得到嘅 theme 掣）,
唔係嘅話下次有人靜靜雞攤返開都冇人知。205 → 220 → **240 條**。
