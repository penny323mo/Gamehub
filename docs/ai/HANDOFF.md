...
Work branch: `main`
Status: Elden Ring II 無敵幀輪唔到出場 (ADR-159)；**MOBA balance 仍然紅**
...
### Elden Ring II：無敵幀根本輪唔到出場 (ADR-156–159)

- **閃避 (ADR-159)**：二十四條檢查冇一條掂過佢。量後果（每秒**郁動時間**掉幾多血）：企定 **8.89–9.02
  %/秒**、不停碌 **1.78**，少八成。**但突變唔肯響**——剷走 `invincibleUntil` 嗰 0.52 秒讀到 1.77。
  拆開兩個機制逐個量：
  ...
  橫睇：無敵幀單獨拎出嚟**有用**（少一半）。直睇右邊：加喺位移上面**一蚊都唔值**。碌一下行 8.4 米而
  雜兵攻擊距離 1.82 米，你一早唔喺度。左上角印證地板：原地碌又冇無敵幀 = 唔碌。條 gate 改咗名，誠實
  講佢守嘅係**位移**，張表寫咗喺測試旁邊。**遊戲一個字都冇改**——憑「喺呢度量唔到」就刪機制，同憑一
  個 bot 嘅死去調平衡係同一個錯。**第八次「綠得唔啱理由」。**
- **職業覆蓋 (ADR-158)**：之前每條檢查都揀 OATHBOUND，三個職業兩個冇載入過（投射物、射程 16／18 米對
  近戰 4.4）。而家有精簡逐職業檢查；反方向令佢哋用近戰嗰組數，兩個都讀到 3.61 米。順帶 clean
  negative：**所有動畫 clip 名都搵得到**（直接讀 GLB 嘅 JSON chunk 對過）。
  要係結構性嘅**，而家一律拆走再重建）；第二版仲係綠，因為只數標住 `fog-gate` 嗰啲而流浪牆冇 tag。
  順手照出第三個：`staticBoxes` 只加唔減。
- **會玩嘅 bot (ADR-157)**：`tests/playthrough.mjs`（**唔喺快速套件**）印 bot 去到邊，**唔判斷**。第一
  關 26–28 秒清到、第二關殺到 **2/3** 先死。兩個假設量完**兩個都錯**（分離力冇事，中位 3.09 米；bot
  喺 2.6 米碌開而射程 4.4 米）。**冇調過任何嘢。**
...
- `node tests/hub.mjs` → **96/96**；`node games/elden-ring-ii/tests/hud-layout.mjs` → **32/32**（五個
