...
- 「量咗但冇 assert」用喺 `sim.mjs` 空手而回，改用突變測試。**第一批完全冇價值**：十四個邊界突變全部
  生還，但嗰啲係**等價突變**——結果講緊我揀嘅算子。第二批（數值放大）**12 個生還 11 個**，包括脫戰
  回血減半、箭速一倍、索敵半徑一倍、塔半徑一倍。
...
- 換算子再噴一次：反轉 `if` 條件 **12 個殺死 8 個**。四個生還者已補 T37–T39：拆塔嘅錢派畀邊隊（反轉
  之後**派畀掉咗塔嗰隊**）、時限打和嘅**第二層決勝**（呢條分支從來冇行過）、同範圍技有冇界住半徑
  （唔守半徑嘅範圍技即係全場技）。
- `view.js` 從來冇噴過，而 browser 一跑十分鐘，所以唔廣噴——揀咗 **ADR-127 自己聲稱嘅兩件事**（一份
  ADR 聲稱一件冇人驗過嘅嘢係最差嗰種文件）：拆走 3 米瞬移門檻 → 高刷單幀跳動肥；拆走鏡頭平滑 →
  打直取景肥。兩個都有守，而後者係畀上一輪先修好嗰條 framing gate 捉到，賺返兩次。
