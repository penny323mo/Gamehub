...
Work branch: `main`
Status: Elden Ring II 死亡畫面從來冇量過版位 (ADR-160)；**MOBA balance 仍然紅**
...
### Elden Ring II：死亡畫面從來冇量過版位 (ADR-156–160)

- **死亡畫面 + 支尺報咗個假陽性 (ADR-160)**：五個尺寸嘅重疊檢查全部喺第一關嗰陣做，所以「YOU DIED」
  呢一幅——每個玩家一定見到，而且係喺最唔想見到亂版嗰刻——由頭到尾冇量過。套件本來就會為咗 ADR-156
  特登死一次，所以喺撳 R 之前重量五個尺寸幾乎零成本。佢五個尺寸全部報重疊，而**係我支尺錯**：
  `<button>` 同佢入面嗰個 `<kbd>R</kbd>`——「有仔就跳過」特登唔包 BUTTON（掣係要量嘅撳擊區），結果
  父仔必然相撞。而家兩邊都排除包含關係。**收窄一支尺最容易靜靜哋整盲佢**，所以驗返佢本來要捉嗰個缺陷
  （放返寫死 `top: 45px`，照樣報 `VEIL × ⚔ 45×8`），而且連死亡狀態都報埋。**第九次「唔啱理由」，
  亦係第一次紅得唔啱理由。**
- **音效名全部搵得到**（clean negative）：十七個定義全部有人叫，所有叫過嘅名全部有定義（十個字面名加
  七個由職業三元式揀）。同 ADR-158 掃動畫嗰次一樣結果。
- **閃避 (ADR-159)**：量後果（每秒**郁動時間**掉幾多血）：企定 **8.89–9.02 %/秒**、不停碌 **1.78**。
  **但突變唔肯響**——剷走無敵幀讀到 1.77。拆開兩個機制：冇位移冇無敵幀 9.05／冇位移有無敵幀 4.67／
  有位移冇無敵幀 1.77／兩樣都有 1.78。即係**無敵幀單獨有用（少一半），但加喺位移上面一蚊都唔值**——
  碌一下行 8.4 米而雜兵攻擊距離 1.82 米。條 gate 改咗名，誠實講佢守嘅係**位移**。**遊戲一個字都冇改。**
- **職業覆蓋 (ADR-158)**：之前每條檢查都揀 OATHBOUND，三個職業兩個冇載入過。而家有精簡逐職業檢查。
  順帶 clean negative：**所有動畫 clip 名都搵得到**。
- **重開會起一道隱形牆 (ADR-156)**：霧門 collider 寫咗**兩次**。條 gate **加強咗兩次先捉到**（第一版
  綠因為 `restart()` 只喺 `if (!bossGateBody)` 先重建，**修法都要係結構性嘅**；第二版仲綠因為只數標住
  `fog-gate` 嗰啲）。順手照出第三個：`staticBoxes` 只加唔減。
- **會玩嘅 bot (ADR-157)**：`tests/playthrough.mjs`（**唔喺快速套件**）印 bot 去到邊，**唔判斷**。兩個
  假設量完**兩個都錯**（分離力冇事；bot 喺 2.6 米碌開而射程 4.4 米）。**冇調過任何嘢。**

### 上一輪：時鐘／弧線／撲擊／路標／北面聖所 (ADR-150–155)

- **兩把鐘 (ADR-150)**：`now` 係原始 `performance.now()`（戰鬥計時器掛喺佢），`delta` 夾住 0.05 秒。
  代價係**跌穿 20 fps 之後郁動慢過真實時間而戰鬥計時器唔會**。CPU 節流實測每秒郁動時間出手 **1× 2.33、
  6× 2.90**，而常數寫住最多 0.71/秒，即**三到四倍**。條 gate 釘喺**常數**唔係另一次量度。
- **揮擊弧線畫緊另一把刀 (ADR-151)**：弧線半徑 1.1–2.0 米跨 **243°**，判定係向前 **4.4 米**錐角 **33°**
  ——**收埋一半以上射程**，**角度多報十四倍**。而家由判定嗰組常數計出。
- **Boss 第二階段本來只係數字大咗 (ADR-152)**：而家開**撲擊**，**預警圈畫喺落點唔係畫喺 boss 身上**。
  **目標喺六十米外冇嘢指住 (ADR-153)**：超過 25 米亮光柱，**兩個方向都守**。
- **霧門開咗之後通去嘅係同一塊地 (ADR-154)**：而家北面有走廊同聖所。連通性 gate 第一版用門檻
  `z > -24` 分霧門同真牆，而圓場牆喺 -22.35 一樣過——**一條門檻分唔開兩件企喺同一個位嘅嘢**，改用 tag。
  **分區補光 (ADR-155)**：兩盞燈射程 34／42 但 three.js 照樣放入 shader 燈迴圈，熄咗 **2.0 → 2.3 fps**。
...
- **Penny 報嘅「技能 CD 卡住」**：兩個冷卻都寫喺「用緊佢嗰條路徑」入面，搬咗上 `step()` loop 頂。
  **但個修正令遊戲難咗**：閒置嘅塔本來留住舊冷卻，而家即刻開火。24 局差幅 34 → **45**，**ironhulk
  17%** 低過 20% 線。啲數講嘅係**近戰一直被呢個 bug 保護緊**。**Hub 兩格一直係 404**：`.gitignore`
  排除咗兩個 `dist/`，而呢個倉係靜態 Pages，**`dist/` 本身就係交付物**。
...
- `node tests/hub.mjs` → **96/96**；`node games/elden-ring-ii/tests/hud-layout.mjs` → **33/33**（五個
