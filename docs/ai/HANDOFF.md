...
- 簡易模式 (the default) no longer pins the throttle at 1.0: `1 - 0.4·|steer|` (ADR-080). Full
  ...
  and full lock took 107 m of travel, now 72 m — on a 15 m road, wall versus corner.
- Grazing a wall no longer ends the race (ADR-081): a 10° contact took 108 km/h to 0. Only the
  along-wall component is scaled (0.97) and the heading turns 25% toward the tangent; the graze
  now bottoms at 34 km/h and is back to 70 in three seconds.
...
- Drifting is now a cornering technique (ADR-085). The old "keep 70% of speed" floor divided by
  the straight-line terminal speed (186 km/h), which no corner reaches, so any real drift failed
  it. Against the grip speed for the same radius: `driftPower` 0.30 held 1.7s on a 147 m radius
  at 69%, 0.38 holds 13.8s on a 52 m radius at 94% — a line three times tighter for 6% of speed.
  Power oversteer is suspended 1.2s after a wall hit, restoring graze recovery (44 → 94 km/h).
...
- Suites: race 119/119, setup 125/125, rivals 61/61, ghost 29/29, season 55/55,
  audio 32/32 (421/421); `run-all` green. AI 0–0.3% off-road on all six circuits. Hub 33/33.
