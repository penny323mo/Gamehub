...
Prepared by: Codex — Racing drift route, wheel-flicker fix and Ashen mobile setup
Integration branch: `main`
Work branch: `main`
Status: source changes are locally verified; commit/push this checkpoint before
the next agent edits the shared checkout.
...
drifts, and the rigid GLB wheel animation no longer produces shared-normal
specular flicker.
...
- `games/Racing Car/tests/setup.mjs` — shared-normal regression gate.
...
- Racing `race.mjs`: **136/136**.
- Racing `setup.mjs`: **158/158** (turbo clearance 53.2m; all six starts,
...
1. Run `./scripts/agent-context.sh --sync`, read this file plus ADR-306/307,
...
4. For future Racing feel work, test the real HTTP page at 844×390 and a
   320×568 viewport; keep the drift scoring/effects and mobile touch gates.
