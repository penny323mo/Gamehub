...
- 「量咗但冇 assert」用喺 `sim.mjs` 空手而回，改用突變測試。**第一批完全冇價值**：十四個邊界突變全部
  生還，但喺浮點時間度嗰啲係**等價突變**——結果講緊我揀嘅算子。第二批（數值放大一倍）**12 個生還
  11 個**，包括脫戰回血減半、箭速一倍、索敵半徑一倍、塔半徑一倍。
- 但答案唔係逐個常數釘死——嗰種就係「記錄實作而唔係記錄意圖」。要守嘅係後果：企喺安全位由半血養返
  滿要 **199.7／242.5／267 秒**（1／6／12 級），而一場波平均八分鐘——「等返血」根本唔係選項。
  T33 用闊帶（120–360 秒）釘住，`/5` 改 `/10` 三個等級全部肥。
- 其餘生還者逐個嚟，守後果唔守常數，兩個方向驗過。索敵半徑 →「行到幾埋先畀小兵盯上」＝ **18 米**，
  T34 釘 12–24。追蹤彈速 →「出手到中招嗰個間隔」＝最遠射程 **0.267 秒**，T35 釘 0.18–0.45。塔半徑
  →「近戰拆塔企離塔心幾遠」＝ **4.19 米**，T36 釘 3.5–5.2（第一次寫 3–5 只捉到一個方向）。
- 換算子再噴一次：反轉 `if` 條件（唔會出等價突變）**12 個殺死 8 個**。四個生還者都係實嘢，已補
  T37–T39：拆塔嘅錢派畀邊隊（反轉之後**派畀掉咗塔嗰隊**）、時限打和嘅**第二層決勝**（呢條分支從來
  冇行過）、同範圍技有冇界住半徑（一個唔守半徑嘅範圍技即係全場技）。三個突變而家全部死。
