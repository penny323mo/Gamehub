...
Work branch: `main`
Status: Elden Ring II 隱形牆 + 會玩嘅 bot (ADR-156/157)；**MOBA balance 仍然紅**
...
- **一個會玩嘅 bot，同兩個查完冇事嘅假設 (ADR-157)**：`tests/playthrough.mjs`（**唔喺快速套件**）行一
  個識行識斬嘅 bot，印佢去到邊，**唔判斷**——bot 死咗唔證明隻遊戲通唔到。第一關（兩隻）26–28 秒清到，
  第二關（三隻）殺到 **2/3** 先死；係難度參考線唔係合格線。兩個假設量完**兩個都錯**：一、死亡瞬間三隻
  埋成一嚿，以為分離力失效——29 個樣本中位 **3.09 米**、**0%** 細過 0.9（貼實係 0.80），我攞咗死嗰一格
  當常態。二、第一個 bot 喺 2.6 米碌開而射程係 4.4 米，即一到打得到就碌走，八十九秒零殺——同 ADR-153
  讀錯 `Space` 一樣，冇睇清楚遊戲自己講咗嘅數。**冇調過任何嘢。**

### 上一輪：時鐘／弧線／撲擊／路標／北面聖所 (ADR-150–155)

- **兩把鐘 (ADR-150)**：`now` 係原始 `performance.now()`（戰鬥計時器掛喺佢），而 `delta` 夾住 0.05 秒
...
- **霧門開咗之後通去嘅係同一塊地 (ADR-154)**：boss 本來喺同一個圓場。而家北面有走廊同半徑 20 嘅聖所。
  連通性 gate 第一版用門檻 `z > -24` 分霧門同真牆，而圓場牆喺 -22.35 一樣過——**一條門檻分唔開兩件企喺
  同一個位嘅嘢**，改用 tag。**分區補光 (ADR-155)**：兩盞燈射程 34／42 但 three.js 照樣放入 shader 燈
  迴圈，熄咗 **2.0 → 2.3 fps** 而 draw call 冇變；而家遠過燈自己個 `distance` 就熄。
