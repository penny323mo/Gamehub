...
- ADR-131's shape — a measurement resting on what the measurement itself changed — aimed at the
  ...
  at the moment of the attack it is at **x = -62**, both sizes. Clearing the standing attack
  order left by an earlier gate changed nothing — the champion **dies during the 25 s warm-up
  and respawns at its own fountain**, a fact ADR-126 recorded without noticing it relocates the
  fixture. Every swing, spark and projectile was measured inside the fountain — the exact state
  ADR-119 named unrepresentative, green for the wrong reason a third time in the same block.
- Fixed by ordering: warm up first, then state the fixture (orders cleared, position, health,
  level), then measure, with `atFountain` asserted false at measurement time — so the gate's
  claim about its own state is checked instead of assumed.
...
- Gold: **74.4% of match time a champion holds enough to buy something while the build list
  forbids it** (avg 1122). A full build costs 8502, a champion earns 4191 a match — **0/72 could
  complete one**. `nextPurchase` now scans to the next affordable item within the same build:
  idle time → 64.0%, gold carried 880 → 577. ADR-130.
- Champion spread was 66 points (longshot 83, duskblade 17) and tracked **range** almost exactly;
  now **34** (dawnkeeper 63, duskblade 29) after mechanism-tied changes: melee speed → 7.1/7.4/7.1,
  longshot range 10.4 → 9.6, dawnkeeper armour cut and its Q's reach 12 → 9.5 (it out-ranged every
  attack in the game while damaging **and** healing on one 7 s button), duskblade's only mobility
  8 s → 5 s. Melee die on the way in: **9.6** deaths a match against 1.3–4.3 ranged. ADR-130.
- **The yardstick was one of the things being tuned** (ADR-131): buffing melee armour made the
  game *less* even (34 → 66), because ironhulk is a companion in every measured match — both teams
  changed at once and the biggest mover was untouched dawnkeeper (63 → 83). Reverted, verified
  byte-level; the same flaw affected the earlier pass, so **"66 → 46" was never a clean
  before/after**. `tests/balance.mjs` sits outside the fast suite and refuses to judge below 24.
- **Every layout gate began after `#pick-go`**: on short screens the pick grid's visible height
  ...
  the fold were scrolled out, not lost, and counting after scrolling flatters. ADR-129.
...
- `node games/moba/tests/browser.mjs` → **187/187 pass** at five sizes (~10 min; `PW_CHROMIUM`
