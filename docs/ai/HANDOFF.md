...
Work branch: `main`
Status: Elden Ring II 開始量像素 (ADR-162)；**MOBA balance 仍然紅**
...
### Elden Ring II：開始量像素 + 地圖成環 (ADR-161/162)

- 全部版位檢查都係量矩形，而矩形對「鏡頭埋咗入牆」完全盲——呢個 session 撞過兩次，兩次張牆表都話鏡頭
  同角色之間乜都冇。所以直接抽 framebuffer 嘅一橫帶，計亮度／標準差／色數。三個狀態實測：
...
- **第一版淨係用標準差，唔肯響**：熄晒燈亮度跌到 10.3，但標準差反而**升**到 21.9——接近全黑但有幾點亮
  嘅畫，變化係大嘅。**對比度唔等於亮度，一個數擔唔起兩種失效。** 而家兩條各守一個：**色數 ≥ 120** 捉
  「一浸平色」、**亮度 ≥ 18** 捉「實質全黑」。兩條驗過兩個方向而且係交叉嘅。守唔到乜亦寫咗喺測試旁邊
  （兩條都抽出生點，捉唔到走廊嗰兩單）。**第十次「唔啱理由」，第二次係指標錯唔係代碼錯。**
- **地圖成環 (ADR-161)**：西面庭院本來係我起出嚟嘅死路（要重行一百二十米）。而家庭院開北口、聖所開
  西口、中間一條 L 形通道。拐角第一次砌錯（兩段牆各自穿過對方條走廊），連通性 gate 影相前就捉到。
  同位置嘅 `wall.glb` 網格厚好多——鏡頭合法噉停喺盒外面半米，已經喺視覺牆入面。pad 0.55 → 1.35 米，
  **通用修正**。

### 上一輪：死亡畫面、閃避、職業覆蓋 (ADR-156–160)

- **死亡畫面從來冇量過版位 (ADR-160)**：五個尺寸嘅重疊檢查全部喺第一關做。加咗之後全部報重疊，而**係
  我支尺錯**：`<button>` 同佢入面嘅 `<kbd>R</kbd>`——「有仔就跳過」特登唔包 BUTTON。而家排除包含關係，
  並驗返佢本來要捉嗰個缺陷仲捉得到。同輪 clean negative：**所有音效名同動畫 clip 名都搵得到**。
- **閃避 (ADR-159)**：企定 **8.89–9.02 %HP/秒**、不停碌 **1.78**，但**剷走無敵幀讀到 1.77**。拆開量：
  冇位移冇無敵幀 9.05／冇位移有無敵幀 4.67／有位移冇無敵幀 1.77／兩樣都有 1.78。**無敵幀單獨有用，但
  加喺位移上面一蚊都唔值**（碌一下行 8.4 米而雜兵攻擊距離 1.82 米）。條 gate 誠實講佢守嘅係**位移**。
- **重開會起一道隱形牆 (ADR-156)**：霧門 collider 寫咗**兩次**，條 gate **加強咗兩次先捉到**；順手照出
  `staticBoxes` 只加唔減。**職業覆蓋 (ADR-158)**：三個職業兩個從來冇載入過。**會玩嘅 bot (ADR-157)**：
  印去到邊，**唔判斷**；兩個假設量完**兩個都錯**，**冇調過任何嘢**。
...
- **揮擊弧線畫緊另一把刀 (ADR-151)**：畫 **243°**、判定 **33°**。**Boss 第二階段本來只係數字大咗
  (ADR-152)**：而家開撲擊，**預警圈畫喺落點**。**目標喺六十米外冇嘢指住 (ADR-153)**：亮光柱，
  **兩個方向都守**。
- **霧門開咗之後通去嘅係同一塊地 (ADR-154)**：連通性 gate 第一版用門檻分霧門同真牆而分唔開——**一條門
  檻分唔開兩件企喺同一個位嘅嘢**，改用 tag。**分區補光 (ADR-155)**：兩盞燈射程 34／42 但 three.js 照樣
  放入 shader 燈迴圈，熄咗 **2.0 → 2.3 fps**。
...
- **玩家郁唔到嘅時間** (ADR-141)：曲線拉平三成半，總時間 **150 對 151 秒**——**個掣係死亡頻率唔係計時
  ...
  `if` 12 殺 8，**一句 ADR 唔等於一條守衛**；手砌小兵缺欄位令座標變 **NaN**，而家逐格斷言。
...
- `node tests/hub.mjs` → **96/96**；`node games/elden-ring-ii/tests/hud-layout.mjs` → **36/36**（五個
