...
Work branch: `main`
Status: 深淵之橋 combat-fixture ordering (ADR-132), balance measurement caveat (ADR-131)
...
### 深淵之橋 the combat gate was measuring a champion inside its own fountain (ADR-132)

- ADR-131's shape — a measurement resting on what the measurement itself changed — aimed at the
  suite: 60-odd gates run in sequence on one page, some resetting the player and some not.
  Instrumented, not reasoned about: the combat gate says it stands the champion at **x = -6**;
  at the attack it is at **x = -62**, because it **dies during the 25 s warm-up and respawns at
  its own fountain** — a fact ADR-126 recorded without noticing it relocates the fixture. Every
  swing, spark and projectile was measured inside the fountain, the state ADR-119 named
  unrepresentative: green for the wrong reason a third time in the same block.
- Fixed by ordering: warm up first, then state the fixture, then measure, with `atFountain`
  asserted false at measurement time. The other gate that sets state then lets time run — the
  layout gate's 900 ms wait — **holds** at all three sizes and is now asserted too: two sites
  of one shape, one wrong by 56 m, one right, neither previously checked.

### Earlier checkpoints, in one line each

- Gold: **74.4% of match time a champion holds enough to buy something while the build list
  forbids it** (avg 1122); a full build costs 8502 and a champion earns 4191 a match, so
  **0/72 could complete one**. `nextPurchase` now scans to the next affordable item within
  the same build: idle 64.0%, gold carried 880 → 577. ADR-130.
- Champion spread was 66 points (longshot 83, duskblade 17) and tracked **range** almost exactly;
  now **34** (dawnkeeper 63, duskblade 29) after mechanism-tied changes: melee speed → 7.1/7.4/
  7.1, longshot range 10.4 → 9.6, dawnkeeper armour cut and its Q's reach 12 → 9.5 (out-ranging
  every attack while damaging **and** healing on one 7 s button), duskblade's only mobility 8 s
  → 5 s. Melee die on the way in: **9.6** deaths a match against 1.3–4.3 ranged. ADR-130.
- **The yardstick was one of the things being tuned** (ADR-131): buffing melee armour made the
  game *less* even (34 → 66), because ironhulk is a companion in every measured match — the
  biggest mover was untouched dawnkeeper (63 → 83). Reverted, verified byte-level; the same flaw
  hit the earlier pass, so **"66 → 46" was never a clean before/after**. `tests/balance.mjs`
  sits outside the fast suite and refuses to judge below 24 matches.
- **Every layout gate began after `#pick-go`**: on short screens the pick grid's visible height
  was smaller than one card (78 vs 228 at 568×320) — **zero complete cards**, `max-height: 74vh`
  never binding under flex shrink. Two probe misreadings on the way — cards below the fold were
  scrolled out, not lost, and counting after scrolling flatters. ADR-129.
...
- Bot update order alternates each tick: blue first gave blue 33/72, red first 48/72 (ADR-113);
  draw calls peak at 286/342, not 1311 (ADR-114); portrait spent **83.6% on abyss and water**
  before the camera rotated 90° about Y for **70.1% ground** (ADR-110); `makeRng` used the seed
  directly as xorshift32 state so the **first output averaged 0.007**, and its first consumer is
  a bot's reaction time (ADR-109); level-1 attack pacing was **8.6–12.7 s per minion**, now
  5.1–7.9 (ADR-108); the iPhone SE HP panel hung off both edges (ADR-116) and the **settings
  panel did not fit 568×320** (ADR-124).
...
- iOS: `overflow-y: auto` + `touch-action: pan-y` reads drift as a scroll and synthesises no
  `click`; `src/tap.js` owns "what counts as a tap". ADR-105/106/107.
...
- `node games/moba/tests/browser.mjs` → **190/190 pass** at five sizes (~10 min; `PW_CHROMIUM`
  overrides the bundled Chromium): champion select and post-match layout, full matches, FX and
