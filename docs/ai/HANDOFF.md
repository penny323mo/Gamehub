...
Prepared by: Codex — Racing Car speed cue and crest/valley pass
...
- Closed crest/valley waves are now more legible (`1.78/0.75/0.28` harmonics),
  with track multipliers turbo **1.28**, coast **1.14**, touge **1.40**. Setup
  on touge-rev measures surface **−3.820…+3.820m**, span **7.639m**, maximum
  pitch **0.04883rad**; height/pitch seam at `t=0/1` remains exact.
...
- Speed layer starts at `10 m/s` and reaches a bounded **0.82** opacity cap
  (`70km/h` setup cue **0.3227**); wheel-motion animates four merged wheel
...
- `games/Racing Car/style.css` (bounded mid-speed speed-streak readability)
- `games/Racing Car/src/race.js`, `games/Racing Car/src/driver.js` (cached runtime tangents)
- `games/Racing Car/tests/setup.mjs` (speed-cue, elevation-distance, touch and hot-path gates),
  `games/Racing Car/tests/race.mjs`, `games/Racing Car/tests/ghost.mjs`
- `docs/ai/PROJECT_CONTEXT.md`, `docs/ai/DECISIONS.md` (ADR-289–ADR-297), `docs/ai/HANDOFF.md`
...
- `setup.mjs` — **154/154**; six tracks, graded ribbon/offroad pose, effects,
...
- `ghost.mjs` — **29/29**; ghost remains physics-independent (the new slope load
  ...
  night + four rivals + ghost + effects frame stays below budget at **17 calls / 53,251 tris**.
...
- Aggregate `RACER_TEST_SETTLE_MS=5000 node games/Racing\ Car/tests/run-all.mjs`:
  race **136/136**, setup **154/154**, rivals **61/61**, ghost **29/29**,
  season **55/55**, audio **33/33**.
- `node --check` on changed JS and tests — PASS; `git diff --check` — PASS.
- Real headed root-served smoke after this pass: **844×390** mobile straight/crest
  view at **65km/h**, and **1200×700** desktop touge crest view at **69km/h**;
  both showed road ribbon, kerbs, guardrails, car contact and controls with
  console **0 errors**. Screenshots were temporary QA artifacts only.
...
- The rigid GLB has no wheel bones/clips; re-profile `wheel-motion.js` if the
  ...
  lean at ±`0.032rad`.
- Do not increase wave amplitude beyond the current `1.78/0.75/0.28` harmonics,
  `gradeGravity`, engine output, camera shake or banking without rerunning
  physical gates, mobile budget and a real screenshot.
...
1. Run `./scripts/agent-context.sh --sync`; read this handoff and ADR-297.
