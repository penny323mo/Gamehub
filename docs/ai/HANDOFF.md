...
Work branch: `main`
Status: Elden Ring II 兩把鐘／弧線／boss 撲擊 (ADR-150/151/152)；**MOBA balance 仍然紅**
...
- CPU 節流量每秒**郁動時間**出幾多手：**1× 2.33，6× 2.90——多兩成半**；同常數比更誇（寫住 1.4 秒一下
  即最多 0.71/秒，實際係**設計節奏嘅三到四倍**）。併成一把鐘後 0.62–0.67。
...
- **Boss 第二階段本來只係數字大咗 (ADR-152)**：由頭到尾得一招 Punch，`phase = 2` 淨係改前搖／傷害／
  半徑／速度——玩家打法完全唔使變。`demon.gltf` 有十四段動畫用緊五段，`Jump`／`Jump_Land` 從來冇出現。
  第二階段而家開**撲擊**：起跳鎖死落點、飛過去，**預警圈畫喺落點唔係畫喺 boss 身上**，命中亦係量離
  落點幾遠——要讀嘅嘢唔同咗，退後唔再自動安全。揀招寫成 module 層純函數
  `chooseBossMove(phase, distance, roll)`：headless 要打到 boss 等於要贏晒兩波兵，一條要贏咗場先量到
  嘢嘅 gate 冇人會跑。三條檢查（一階只有拳、二階遠距兩招都出、二階埋身唔撲）都驗過反方向。
