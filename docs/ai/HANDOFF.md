...
Baseline before this task: `ddf6bbf`
...
- `docs/ai/DECISIONS.md` (ADR-045 to ADR-052), `docs/ai/HANDOFF.md`, `src/minimap.js`

## Verification

- `npm test` in `games/Racing Car/tests`: race 47/47, setup 59/59, rivals 32/32,
  ghost 25/25. One run-all had setup fail once and pass standalone and on re-run —
  treat setup as mildly flaky under back-to-back browser launches, not as green-only.
...
3. Natural next step: a season or championship carrying results across races, now
   that rivals have stable identities to accumulate points against.
