...
export class RaiderDrone {
  ...
  constructor(readonly root: TransformNode, readonly variant: DroneVariant, side: number, scene: Scene) {
    ...
    if (variant === "kamikaze") {
      const mat = new StandardMaterial(`danger-${this.id}`, scene); mat.emissiveColor = new Color3(1, 0.08, 0); mat.alpha = 0.65;
      const aura = MeshBuilder.CreateTorus(`danger-aura-${this.id}`, { diameter: 2.2, thickness: 0.08, tessellation: 24 }, scene);
      aura.material = mat; aura.parent = root; aura.rotation.x = Math.PI / 2; this.aura = aura;
    }
  }
  ...
  update(delta: number, player: Vector3, core: Vector3): DroneAttack | null {
    ...
    if (this.variant === "kamikaze") {
      ...
      if (distance < 1.25) { this.dead = true; this.state = "Dying"; return { target: "core", origin: this.root.position.clone() }; }
      return null;
    }
    ...
    if (this.attackTimer <= 0 && distance < 3) { this.attackTimer = GAMEPLAY.drone.attackInterval + Math.random() * 0.8; this.state = this.attackingCore ? "AttackingCore" : "AttackingPlayer"; return { target: this.attackingCore ? "core" : "player", origin: this.root.position.clone() }; }
    return null;
  }

  releaseExtras(): void { this.aura?.dispose(); }
}
