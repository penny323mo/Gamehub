...
export class WorldScrollSystem {
  ...
  constructor(scene: Scene) {
    ...
    for (let index = 0; index < 5; index += 1) {
      const ground = MeshBuilder.CreateGround(`ground-${index}`, { width: 64, height: 32 }, scene); ground.position.set(0, -5.15, -32 + index * 32); ground.material = groundMaterial; this.items.push({ mesh: ground, speed: 17, length: 160 });
      for (const x of [-2.15, 2.15]) {
        const rail = MeshBuilder.CreateBox(`rail-${index}-${x}`, { width: 0.14, height: 0.16, depth: 32 }, scene); rail.position.set(x, -4.98, ground.position.z); rail.material = railMaterial; this.items.push({ mesh: rail, speed: 17, length: 160 });
      }
    }
  ...
  private updateRocks(delta: number): void {
    ...
    for (let index = 0; index < this.rockZ.length; index += 1) {
      ...
      const matrix = Matrix.Compose(new Vector3(scale, scale * 0.7, scale), Vector3.Zero().toQuaternion(), new Vector3(x, -4.65, this.rockZ[index] ?? 0));
