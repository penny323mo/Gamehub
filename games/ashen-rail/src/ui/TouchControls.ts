...
export class TouchControls {
  ...
  constructor(private readonly callbacks: ControlCallbacks) {
    if (!this.joystick || !this.knob) throw new Error("Touch control DOM is incomplete");
    ...
    window.addEventListener("blur", () => { this.keyboard.clear(); this.movement.set(0, 0); this.firing = false; });
  }
  ...
  reset(): void { this.movement.set(0, 0); this.firing = false; this.joystickPointer = undefined; if (this.knob) this.knob.style.transform = "translate(-50%, -50%)"; }
  ...
  private readonly onJoystickMove = (event: PointerEvent): void => { if (event.pointerId === this.joystickPointer) { event.preventDefault(); this.applyJoystick(event); } };
  private readonly onJoystickUp = (event: PointerEvent): void => { if (event.pointerId !== this.joystickPointer) return; this.joystickPointer = undefined; this.movement.set(0, 0); if (this.knob) this.knob.style.transform = "translate(-50%, -50%)"; };
