...
// ---------- T40：一場波入面，玩家有幾多時間係㩒乜都冇反應 ----------
//
// `RESPAWN_BASE + RESPAWN_PER_LEVEL × level` 個名叫「重生時間」，但佢只係
// 玩家等唔到玩嗰段時間嘅一半多啲。真正等嘅係「死咗」加「由泉水行返去」，
// 而後者冇任何常數講過、冇任何測試量過。實測 157 次重生：名義中位 15.2 秒，
// 行返去再多 5.8 秒——**平均喺個名義數之上再加 48%**。同一形狀喺 pace.js
// 出現過（`dropped` 只報自己嗰半），亦係 T10 嘅盲點：T10 只問「有冇重生
// 時間」，答案永遠係有。
//
// **點解呢度一條「總共冇得玩幾多」都冇**（寫過兩版，兩版都掉咗）：
//   第一版守佔比。將重生時間由 8+1.8L 減到 5+0.75L，佔比 26% → 19%「達標」，
//   但同一批局嘅絕對冇得玩秒數係 150 → 161，即係**變差咗**——局數由 9.6
//   拉到 13.9 分鐘，分母自己大咗。
//   第二版改守絕對秒數。反方向驗嗰陣將暮刃血量斬半，冇得玩秒數由 182 跌到
//   91，條 gate 綠燈——因為局數塌到 4 分鐘，隊伍輸得快，自然冇時間死。
// 兩版都係同一個病：一個「總量」跟住局長走，而局長本身就係我改緊嘅嘢。
// 任何用一場波做分母（或者做總量）嘅數，喺呢度都答唔到問題。
//
// 所以三條線各自守一件唔會互相溝淡嘅事：
//   一、最衰嗰次鎖幾耐（最大值，唔可以平均）；
//   二、一局唔准拖長（拖長就係上面嗰條嘅走盞位）。
// 「幾密鎖一次」（死／分鐘）唔喺呢度：局同局之間 0.49 到 1.23，三局分唔到。
// 實測將重生時間減到 6+1.1L——24 局睇係 0.79 → 1.04，一個三成嘅真變化——
// 呢度三局只讀到 0.87 → 0.90，唔會響。要幾多局先分得清就要跑幾多局，
// 所以佢擺咗入 balance.mjs（嗰度本來就跑 24 局，亦本來就寫住樣本唔夠
// 就唔好扮判斷）。一條分唔清嘅 gate 唔係鬆，係假。
//
// 順帶量到嘅嘢（ADR-141）：重生常數根本唔係個掣。24 局實測，8+1.8L 對
// 6+1.1L 嘅絕對冇得玩秒數係 150 對 151——因為死一次嘅時間短咗三成半，
// 一局就死多三成（7.6 → 9.9 次）。冇得玩嘅總時間係**死亡頻率**嘅不動點，
// 唔係計時器嘅。要郁呢個數，要郁嘅係近戰點解一局死九次（ADR-130）。
{
    const 最長 = [], 分鐘 = [];
    for (const seed of [1, 2, 3]) {
        ...
        let t = 0, 總 = 0, 死 = 0, 段 = 0, 長 = 0, 生 = true;
        while (!sim.over && sim.time < GAME_MAX) {
            updateBots(bots, TICK, t); sim.step(TICK); t++;
            總 += TICK;
            // 「行返去」幾時完：見到敵方英雄十五米以內就當返到場。用敵人做
            // 準繩，唔用座標，因為戰線成場都喺度郁——用固定座標就會變成
            // 「量緊地圖」而唔係「量緊玩家等咗幾耐」。
            const 返到 = 我.alive && sim.champions.some(o => o.team !== 我.team
                && o.alive && Math.hypot(o.x - 我.x, o.z - 我.z) < 15);
            if (!我.alive || (!生 && !返到)) { 死 += TICK; 段 += TICK; 生 = false; continue; }
            長 = Math.max(長, 段); 段 = 0; 生 = true;
        }
        長 = Math.max(長, 段);
        最長.push(長); 分鐘.push(總 / 60);
    }
    // 三條線都係棘輪：釘喺實測到嘅最差值再留水位，唔係一個理想值。意思係
    // 「唔准變差」，唔係「已經夠好」——一局鎖住成三分鐘仍然係多，而點樣
    // 減落去係 ADR-130 嗰條平衡軸嘅事，唔係呢度改得掂。
    const 單次 = Math.round(Math.max(...最長));
    const 最長局 = +Math.max(...分鐘).toFixed(1);
    check('單次冇得玩唔可以連續超過 40 秒',
        單次 <= 40, { 最長秒: 單次, 三局: 最長.map(v => Math.round(v)) });
    check('一局唔可以拖過 16 分鐘（唔准用拉長局數嚟溝淡上面兩條）',
