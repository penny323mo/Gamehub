...
//
// Playwright 沿用 games/tower 嗰個安裝——同 root Hub gates 一樣。
...
const PW = path.join(ROOT, 'games', 'tower', 'node_modules', 'playwright', 'index.mjs');
if (!fs.existsSync(PW)) {
...
// 就慢過 sim。所以呢條 gate 唔靠真幀率，而係照 pace.js 嘅算式親手推幀。
{
    ...
    for (const [tag, r] of Object.entries(smooth)) {
        // 三十幀嗰行淨係做基線觀察，唔斷言「郁嘅比例」。喺 sim 速率之下一幀
        // 就係一格，郁唔郁純粹係拍子對唔對齊，唔關順滑度事。之前擺咗個 0.7
        // 落去，係由一次觀察（0.798）嗌出嚟嘅數——角色速度一改就跌到 0.664，
        // 而嗰個改動同順滑度冇任何關係。條 gate 應該問「快過 sim 嘅幀率會唔
        // 會凍」，唔應該問一個由拍子決定嘅數。
        if (tag === '三十') { check(`${tag}：內插冇令角色行少咗路`, r.走咗幾遠 > 1, r); continue; }
        check(`${tag}：畫面逐幀都郁（唔會三幀凍一幀跳）`, r.郁嘅比例 > 0.9, r);
        check(`${tag}：單幀跳動細過一格嘅距離`, r.單幀最大跳動 < 0.12, r);
