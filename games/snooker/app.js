(() => {
  ...
  canvas.addEventListener('pointerdown', (e) => {
    ...
    // Zone-based input logic
    if (state.phase !== 'aim') return;

    const dx = x - state.cue.x;
    const dy = y - state.cue.y;

    // Distance check - FULL TABLE powering support
    // First touch always sets aiming direction, then can drag anywhere to power
    if (!state.positionsConfirmed) confirmPlacement();

    // Set initial aim direction based on touch point
    setAimFromPointer(x, y);

    // Enter powering mode - can drag anywhere on table
